...
Output from ssleuth -h:
    ...
    '$TABLE_NAME'"
      ...
      --column-names-query, -n <s>:   SQL query to use to get column names for a table. Substitute $TABLE_NAME for the table name. e.g. "select column_name FROM all_tab_cols where table_name =
       ...
       --maximum-row-count, -m <i>:   maximum row count. Does not attempt to follow path if more than this many rows are found from a query. However, you can still get more than one row in result
                                      summaries, via multiple queries. (Default: 1000)
