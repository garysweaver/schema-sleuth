...
If debug is not specified, you can see progress via the dots, etc. printed. A '.' means that data was retrieved. A 'M' indicates results exceeded the maximum count, so that path was cut. A 'O' indicates that the maximum number of steps in the path was exceeded, so that path was cut.

Executing with the -d switch provides greater debugging information.
