...
== Usage
     ...
     --column-names-query, -n <s>:   SQL query to use to get column names for a table. Substitute $TABLE_NAME for the table name.
                                     e.g. "select column_name FROM all_tab_cols where table_name =
                                '$TABLE_NAME'"
...
Custom:
      ...
      ssleuth -r DBI:OCI8://db.acme.org:1234/ACMETEST.WORLD -u jdoe -p secret -c user_id -e 1234 \
       ...
       -n "select column_name FROM all_tab_cols where table_name = ?"
